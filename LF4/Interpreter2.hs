...
import AbsLF (Ident)
import AbsLFAux
import Control.Monad.State (MonadState (get), StateT, lift, runStateT)
import Prelude hiding (lookup)

executeP :: Program -> (Valor, Enviroment)
executeP (Prog fs) =
  case runStateT (eval (expMain fs)) initialEnv of
    Just (val, finalEnv) -> (val, finalEnv)
...
type Eval a = StateT Enviroment Maybe a
...
eval x = case x of
  ...
  EDiv exp0 exp -> do
    ...
    v2 <- eval exp
    if i v2 == 0
      then lift Nothing -- Divisão por 0
      else return (ValorInt (i v1 `div` i v2))
  EOr exp0 exp -> do
    v1 <- eval exp0
    v2 <- eval exp
  ...
  EIf exp expT expE -> case eval exp of
    (ValorInt v) ->
      if v /= 0
        then eval expT
        else eval expE
  ...
  ECall id lexp -> do
    -- A. Evaluate arguments in current scope
    argValues <- mapM eval lexp
    (currentCtx, currentMem) <- get
    case lookupECallLog currentMem id argValues of
      Just val -> return val -- Cache Hit!
      Nothing -> do
        let (ValorFun funDef) = lookup currentCtx id
        let parameters = getParams funDef

        -- Contexto para chamada
        let contextFunctions = filter (\(_, v) -> case v of ValorFun _ -> True; _ -> False) currentCtx
        let paramBindings = zip (map Decl parameters) argValues
        let newContext = paramBindings ++ contextFunctions

        
        put (newContext, currentMem)
        resultVal <- eval (getExp funDef)

        (_, memAfterBody) <- get

        let finalMem = updateECallMem memAfterBody id argValues resultVal

        put (currentCtx, finalMem)
